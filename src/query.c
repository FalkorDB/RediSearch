  ...
  switch (n->type) {
    ...
    case QN_PREFIX:
      return Query_EvalTagPrefixNode(q, idx, n, iterout, weight,
                         FieldSpec_HasSuffixTrie(fs), caseSensitive);
...
static IndexIterator *Query_EvalMissingNode(QueryEvalCtx *q, QueryNode *qn) {
  ...
  if (!FieldSpec_IndexesMissing(fs)) {
    QueryError_SetWithUserDataFmt(q->status, QUERY_EMISSING,
                           "'ismissing' requires field to be defined with '" SPEC_INDEXMISSING_STR "'", ", field: '%s'",
                           qn->miss.fieldName);
    return NULL;
  }
